...
public class GreetingController {
    ...
    Greeting konnichiwa(@RequestParam(value = "name",
            defaultValue = "konchiwa~") String name) {
        Greeting greeting = new Greeting(new GreetId(counter.incrementAndGet()), new Name(name));
        return greeting;
    }

    @RequestMapping(value = "chiwa",
            method = RequestMethod.GET)
    Greeting chiwa(@Validated
    @RequestParam(value = "name",
            defaultValue = "anonymous") GreetingRequest greetingRequest) {
